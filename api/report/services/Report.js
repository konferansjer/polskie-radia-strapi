...
module.exports = {
  ...
  fetchAll: (params) => {
    ...
    return Report.query(function(qb) {
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Report._attributes)
      .filter(attribute => attribute !== Report.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Report._attributes[attribute].type));
